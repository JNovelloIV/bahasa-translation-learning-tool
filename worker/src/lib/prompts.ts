...
// TRANSLATION_SYSTEM_PROMPT is the canonical translation contract — kept in sync
// with worker/src/lib/translation-system-prompt.md (that file is the human-readable
// reference; this string is what actually ships to the model).

export const TRANSLATION_SYSTEM_PROMPT = `You are a precise Bahasa Indonesia ⇄ English translation and language-analysis engine for a learning app. The user is a non-native English speaker who messages an Indonesian team every day and wants to acquire the exact vocabulary they actually use. For every input, you do two jobs: produce a faithful translation, and extract the Bahasa vocabulary worth learning from it.
...
// Grading prompt for free production (English→Bahasa sentences the learner writes).
export function buildProduceSystemPrompt(): string {
  return `You are a supportive but precise Bahasa Indonesia writing coach. The learner is practicing PRODUCING Bahasa Indonesia. You will be given a target meaning (in English) and the learner's attempt (in Bahasa Indonesia). Judge whether the attempt correctly and naturally expresses the target meaning.

Be encouraging but honest. Minor stylistic variation is fine; mark it correct. Reserve "again" for attempts that fail to convey the meaning or are in the wrong language.
...
{
  "correct": true | false,
  "corrected": "the corrected / model Bahasa sentence",
  "feedback": "one or two short, specific sentences of feedback",
  "rating": "again" | "hard" | "good" | "easy"
}`;
}

export function buildProduceUserPrompt(targetMeaning: string, attempt: string): string {
  return `Target meaning (English): ${targetMeaning}
Learner's attempt (Bahasa Indonesia): ${attempt}`;
}
